...
export function AdminPanelShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="landing-page relative h-[100dvh] w-full overflow-hidden bg-[#03060c]">
      {/* Premium minimal background system */}
      <div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-cyan-900/10 via-[#03060c] to-[#03060c] hidden lg:block" />
          ...
          <div className="flex h-full flex-col p-5">
            ...
            <div className="mb-10 flex items-center gap-3.5 px-3 pt-4">
              ...
              <div className="flex flex-col justify-center">
                <p className="text-[0.55rem] font-bold uppercase tracking-[0.35em] text-cyan-400/70">DIU LENS</p>
                ...
                <div key={section.title} className="flex flex-col gap-1.5">
                  ...
                  {section.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <span className={cn('transition-colors', isActive ? 'text-cyan-400' : 'text-slate-500 group-hover:text-slate-400')}>{item.icon}</span>
                        {item.label}
                      </Link>
                    );
                  })}
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col bg-[#060913] border-r border-white/[0.04] pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] lg:hidden shadow-2xl"
            >
              <div className="flex items-center justify-between border-b border-white/[0.04] px-5 py-4 min-h-[60px]">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[0.8rem] font-bold tracking-widest text-cyan-400">DIU LENS</span>
                </div>
                <button
                  type="button"
                  onClick={() => setIsMobileMenuOpen(false)}
                        ...
                        return (
                          <Link
                            ...
                            className={cn(
                              ...
                              isActive ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-300 active:bg-white/[0.05]'
                            )}
                          >
                            <span className={cn(isActive ? 'text-cyan-400' : 'text-slate-500')}>{item.icon}</span>
                            {item.label}
                          </Link>
                        );
                      })}
