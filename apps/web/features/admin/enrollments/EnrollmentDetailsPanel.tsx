  ...
  if (!imageUrl) {
    return (
      <div className={`flex items-center justify-center bg-slate-900 border border-slate-800 ${className || ''}`}>
        <div className="h-4 w-4 animate-spin rounded-full border-2 border-slate-800 border-t-[#8BB8D0]"></div>
      </div>
    );
  }
        ...
        {studentId && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 right-0 z-50 w-full sm:w-[90vw] max-w-5xl border-l border-slate-800 bg-[#111318] shadow-2xl flex flex-col pt-[env(safe-area-inset-top)]"
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-white/[0.04] bg-[#111318]/80 backdrop-blur-md p-4 sm:p-6 sticky top-0 z-10 min-h-[64px]">
                <div>
                  <h2 className="text-[1.05rem] sm:text-xl font-semibold text-slate-100 flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4 sm:h-5 sm:w-5 text-[#8BB8D0]" />
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-8 relative pb-[calc(140px+env(safe-area-inset-bottom))]">
                {loading ? (
                  <div className="flex items-center justify-center h-40">
                    <div className="h-8 w-8 animate-spin rounded-full border-4 border-slate-800 border-t-[#8BB8D0]"></div>
                  </div>
                ) : error ? (
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-2xl bg-black/20 p-3 sm:p-4 border border-white/[0.03]">
                          ...
                          <div className="flex items-end gap-2">
                            <span className="text-xl sm:text-2xl font-semibold text-[#8BB8D0]">
                    ...
                    <div className="order-3 lg:order-none lg:col-start-1 lg:row-start-1 lg:row-span-5 space-y-5 sm:space-y-6">
                      <div className="flex items-center justify-between">
                        ...
                        {details.biometric_diagnostics.overall_capture_quality >= 75 ? (
                          <span className="flex items-center gap-1.5 text-[0.65rem] sm:text-xs font-medium text-[#8BB8D0] bg-[#8BB8D0]/10 px-2.5 py-1 rounded-full border border-[#8BB8D0]/20">
                            <span className="h-1.5 w-1.5 rounded-full bg-[#8BB8D0]"></span> Excellent
                          </span>
                      ...
                      <div className="relative pl-5 space-y-6 before:absolute before:inset-y-2 before:left-[9px] before:w-px before:bg-white/[0.08]">
                        {details.timeline.map((log, i) => (
                          <div key={log.id} className="relative">
                            <div className={`absolute -left-7 h-3 w-3 rounded-full border-2 border-[#03060c] ${i === 0 ? 'bg-[#8BB8D0] shadow-[0_0_10px_rgba(139,184,208,0.5)]' : 'bg-slate-700'}`} />
              ...
              <div className="absolute bottom-0 inset-x-0 border-t border-white/[0.04] bg-[#111318]/90 backdrop-blur-xl p-4 sm:p-6 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:pb-[calc(1.5rem+env(safe-area-inset-bottom))] z-20">
